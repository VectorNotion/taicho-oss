...
export function ProspectCard({ prospect, compact = false }: ProspectCardProps) {
  ...
  const company = prospect.company as string | null;
  const title = prospect.title as string | null;
  const email = prospect.email as string | null;
  const linkedinUrl = prospect.linkedinUrl as string | null;
  const status = prospect.status as string | null;
  const lastContactedAt = prospect.lastContactedAt as string | null;
  const hasResearch = prospect.hasResearch as boolean | null;
  const research = prospect.research as {
    industry?: string;
    companySummary?: string;
    talkingPoints?: string[];
    outreachAngle?: string;
  } | null;
  const companyInsights = prospect.companyInsights as Array<{ category: string; content: string }> | null;
  const outreachMessages = prospect.outreachMessages as Array<{
    id: string;
    medium: string;
    subject?: string;
    content: string;
  ...
  const activities = prospect.activities as Array<{ id: string; type: string; title: string; notes?: string; createdAt?: string }> | null;
  const notes = prospect.notes as Array<{ id: string; content: string; createdAt?: string }> | null;
  const qualification = prospect.qualification as { score?: number; matchedPersonaName?: string; notes?: string } | null;
  ...
  if (compact) {
    return (
      <div className="p-3 rounded-lg border hover:bg-muted/50 transition-colors" data-component="DATA-01 Prospect Result Card">
  ...
  return (
    <Card className="my-2 rounded-lg" data-component="DATA-01 Prospect Result Card">
