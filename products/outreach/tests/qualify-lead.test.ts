/**
 * DI-stubbed unit tests for the dimension-based qualify-lead orchestrator
...
interface Recorder {
  ...
  savedQualifications: Array<Parameters<QualifyLeadDeps['saveProspectQualification']>[1]>;
  priorities: number[];
}
...
function makeDeps(config: {
  ...
  const deps: Partial<QualifyLeadDeps> = {
    getLeadById: async () => (config.lead === undefined ? LEAD : config.lead),
    resolveAccountForLead: async () => (config.account === undefined ? ACCOUNT : config.account),
    ...
    saveProspectQualification: async (_leadId, result) => {
    ...
    updateLeadPriorityByScore: async (_leadId, score) => {
...
test('full run: researches everything, QUALIFIED, emits lead.qualified with scores', async () => {
...
test('missing lead throws', async () => {
  const { deps } = makeDeps({ lead: null });
  await assert.rejects(() => runQualifyLead('missing', deps), /Lead not found/);
});
