...
import { leadInsightSourceTarget } from '../domain/lead-intelligence';

test('transcript citations retain the exact evidence, meeting, and offset target', () => {
  assert.deepEqual(leadInsightSourceTarget({
    id: 'utterance-123',
...
test('manual updates and rich notes resolve to distinct Notes anchors', () => {
  assert.deepEqual(leadInsightSourceTarget({ id: 'update-123', type: 'manual_update' }), {
    tab: 'notes',
    anchorId: 'evidence-update-123',
    recordId: 'update-123',
  });
  assert.deepEqual(leadInsightSourceTarget({ id: 'note:note-456', type: 'note' }), {
    tab: 'notes',
