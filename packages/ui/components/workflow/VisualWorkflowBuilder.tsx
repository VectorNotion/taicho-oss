...
/**
 * Structural view of a node's data contract. Producers (the Automations
 * engine) satisfy this with their richer engine types; the canvas and any
 * caller-rendered panels read only these fields.
 */
export type WorkflowNodeArtifactState = {
  accepts: Array<{ kind: string; label: string; tone: string }>;
  produces: Array<{ kind: string; label: string; tone: string; description: string }>;
  matched: Array<{ kind: string; label: string; sourceLabel: string; fields: Array<{ key: string; label: string }> }>;
  missing: unknown[];
};
import { runtimeStatusLabel, runtimeStatusVariant } from './runtime-ui';

/**
 * One edge system for every canvas (funnel steps and automations): identical
 * width everywhere, semantic tones only. Callers may set `data.tone`; raw
 * `style` on an edge is ignored — the canvas owns every edge pixel.
 */
export type WorkflowEdgeTone = 'active' | 'attention';

const EDGE_STROKE_WIDTH = 1.5;

function edgeStyle(tone: WorkflowEdgeTone | undefined): Record<string, unknown> {
  if (tone === 'active') return { stroke: 'var(--primary)', strokeWidth: EDGE_STROKE_WIDTH };
  if (tone === 'attention') return { stroke: 'var(--destructive)', strokeWidth: EDGE_STROKE_WIDTH, strokeDasharray: '5 4' };
  return { strokeWidth: EDGE_STROKE_WIDTH };
}
...
export type WorkflowBuilderNodeData = {
  ...
  artifactState?: WorkflowNodeArtifactState;
  ...
  return (
    ...
    >
      ...
      <div className="flex min-h-0 flex-1">
        <aside className="w-48 shrink-0 space-y-5 border-r p-3 max-md:hidden">
      ...
      <div className="grid gap-2" key={field.key}>
        ...
        {field.action && (
          <Button asChild variant="link" size="sm" className="h-auto justify-start p-0 text-xs">
            <a href={field.action.href} target={field.action.newTab ? '_blank' : undefined} rel={field.action.newTab ? 'noreferrer' : undefined}>
              {field.action.label}
            </a>
          </Button>
        )}
  ...
  return (
    <>
      <div className={`flex min-h-0 overflow-hidden rounded-lg border bg-background ${className}`}>
        {categories.length > 0 && (
          <aside className="w-48 shrink-0 space-y-5 overflow-y-auto border-r bg-background p-3 max-md:hidden">
        ...
        <div className="relative min-w-0 flex-1" data-testid="workflow-canvas">
          <ReactFlow
            ...
            edges={edges.map((edge) => ({
              ...
              style: edgeStyle((edge.data as { tone?: WorkflowEdgeTone } | undefined)?.tone),
            }))}
            nodeTypes={nodeTypes}
            ...
            // @xyflow/react is MIT — attribution removal is expressly permitted
            // by the license; no Pro subscription is required or implied.
            proOptions={{ hideAttribution: true }}
          >
            <Background gap={22} size={1.2} />
            <Controls position="bottom-left" />
            {nodes.length > 8 ? (
              // Small graphs fit on screen whole; an always-on minimap only
              // covers content. It earns its corner once orientation is real.
              <MiniMap pannable zoomable position="top-right" className="!bg-background/80" />
            ) : null}
          </ReactFlow>
        </div>
